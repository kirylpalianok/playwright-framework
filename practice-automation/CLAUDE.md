...
This is the mandatory engineering standard for every contributor and AI agent (including Claude Code) changing this repository. The repository is a production-grade Playwright and TypeScript framework for UI and API verification, Allure reporting, and GitHub Actions execution. Treat it as a shared internal platform for more than 50 automation engineers: every change must be understandable, safe to extend, and inexpensive to operate without tribal knowledge.
...
Claude Code (and any other AI agent working in this repository) MUST:

- Inspect before editing, preserve unrelated work, and use existing scripts and conventions rather than inventing new ones.
- Make material assumptions explicit before acting on them, and keep a short plan with one active step for multi-step work.
- Diagnose selector, readiness, test-data, environment, and product evidence before changing retries, waits, or assertions.
- State validation limits and never claim a command, test, workflow, or integration passed unless it was actually run and observed to pass in this session.
- Never commit, push, open a pull request, alter remote state, change secrets, or run a destructive command without explicit instruction.
...
src/
  ...
  core/           config, fixtures, observability, runtime, validation
  ui/             Playwright page and component adapters
tests/
  accessibility/  accessibility coverage
  api/            API-level specs
  ui/             UI-level specs
  test-data/      builders and data lifecycle
docs/             architecture notes, ADRs, operations docs
```
